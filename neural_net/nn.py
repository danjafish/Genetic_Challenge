...
class DeepConv(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv1d(in_channels=20, out_channels=32, kernel_size=120)
        self.conv2 = torch.nn.Conv1d(in_channels=32, out_channels=32, kernel_size=60)
        self.conv3 = torch.nn.Conv1d(in_channels=32, out_channels=64, kernel_size=30)
        self.conv4 = torch.nn.Conv1d(in_channels=64, out_channels=64, kernel_size=15)
        self.conv5 = torch.nn.Conv1d(in_channels=64, out_channels=128, kernel_size=7)
        self.conv6 = torch.nn.Conv1d(in_channels=128, out_channels=128, kernel_size=3)
        ...
        self.dense_for_one_hot = torch.nn.Linear(40, 9)
        self.dense = torch.nn.Linear(128*6 + 9, 1314)
    ...
    def forward(self, x, y):
        ...
        x1 = self.bn1(x1)
        x1 = F.max_pool2d(x1, kernel_size=(1, 4))

        x2 = F.relu(self.conv2(x1))
        x2 = self.bn2(x2)
        x2 = F.max_pool2d(x2, kernel_size=(1, 4))

        x3 = F.relu(self.conv3(x2))
        x3 = self.bn3(x3)
        x3 = F.max_pool2d(x3, kernel_size=(1, 2))

        x4 = F.relu(self.conv4(x3))
        x4 = self.bn4(x4)
        x4 = F.max_pool2d(x4, kernel_size=(1, 2))

        x5 = F.relu(self.conv5(x4))
        x5 = self.bn5(x5)
        x5 = F.max_pool2d(x5, kernel_size=(1, 2))

        x6 = F.relu(self.conv6(x5))
        x6 = self.bn6(x6)
        x6 = F.max_pool2d(x6, kernel_size=(1, 4))
        # print('after conv ', x6.shape)
...
class Cnn_seq(torch.nn.Module):
    ...
    def __init__(self, n_channels):
        ...
        self.dense = torch.nn.Linear(n_channels * 5 + 9, 1314)
